...
def train_fn(disc_P, disc_M, gen_M, gen_P, loader, opt_disc, opt_gen, l1, mse, d_scaler, g_scaler, current_dir_name):
    P_reals = 0
    P_fakes = 0
...
def main_with_hyperparameter_loop():
    #epocs_list = np.linspace(100, 1000, 5).tolist()
    #batch_size_list = [1, 4, 16, 32]
    #optim_list = ['adam', 'rmsprop', 'sgd', 'adagrad']
    #learning_rate_list = np.linspace(0.00001, 0.1, 5).tolist()

    epocs_list = [3, 5]
    batch_size_list = [1]
    optim_list = ['adam', 'rmsprop','sgd']
    learning_rate_list = np.linspace(0.00001, 0.1, 2).tolist()

    output_df = pd.DataFrame(columns=['epocs_size', 'current_epoch', 'batch_size', 'opt_disc', 'opt_gen', 'learning_rate', 'G_loss', 'D_loss'])

    for current_epoc in epocs_list:
          ...
          for current_optim_disc in optim_list:
            for current_lr in learning_rate_list:
              ...
              for epoch in range(current_epoc):
                  G_loss, D_loss = train_fn(disc_P, disc_M, gen_M, gen_P, loader, opt_disc, opt_gen, L1, mse, d_scaler, g_scaler, current_dir_name)
